 ...
 * <p>
 * beanFactory子接口，能够自动装配的BeanFactory接口。
 *
...
public interface AutowireCapableBeanFactory extends BeanFactory {
	 ...
	 * <p>
	 * 使用指定自动注入策略初始化bean，
	 * 如果是按构造参数注入会指定InstantiationAwareBeanPostProcessor，
	 * 不会执行beanpostprocessor或bean的任何进一步初始化。
	 *
	 ...
	 * <p>
	 * 将beanpostprocessor应用于给定的现有bean实例，调用它们的postprocessbefore初始化方法。
	 ...
	 * The returned bean instance may be a wrapper around the original.
	 * <p>
	 * 将beanpostprocessor应用于给定的现有bean实例，调用它们的postprocessafter初始化方法。
	 * 返回的bean实例可能是原始bean的包装器。
	 *
	 * @param existingBean the existing bean instance
	 * @param beanName     the name of the bean, to be passed to it if necessary
